...
function preload() {
  ...
  for (let i = 0; i < 4; i++) {
    for (let j = 1; j <= 13; j++) {
      let d = new Card(i, j);
      d.setPos(windowWidth - 70, floor(windowHeight / 2));
      deck.push(d);
    }
  }
...
function setup() {
  ...
  srdceButton = new Button(floor(width / 2) + 30, floor(height / 2) - 30, "./images/srdce.png", false, SRDCE);
  srdceButton.setFunction(changeSuit);
  pikyButton = new Button(floor(width / 2) - 30, floor(height / 2) - 30, "./images/piky.png", false, PIKY);
  pikyButton.setFunction(changeSuit);
  krizeButton = new Button(floor(width / 2) - 30, floor(height / 2) + 30, "./images/krize.png", false, KRIZE);
  krizeButton.setFunction(changeSuit);
  karyButton = new Button(floor(width / 2) + 30, floor(height / 2) + 30, "./images/kary.png", false, KARY);
...
function draw() {
  ...
  for (let i = hand1.length - 1; i >= 0; i--) {
    ...
    c.setPos(53 + i * 32 - j * (width - 50), height - 110 - 130 * j);
  }
  ...
  for (let i = hand2.length - 1; i >= 0; i--) {
    ...
    c.setPos(53 + i * 32 - j * (width - 50), 110 + 130 * j);
  }
  ...
  image(cardback, floor(width - 70) - 2 * deck[0].image.height * 0.026 * 0.6, floor(height / 2) + 2 * deck[0].image.width * 0.026 * 0.6, deck[0].image.width * 0.6, deck[0].image.height * 0.6);
  image(cardback, floor(width - 70) - deck[0].image.height * 0.026 * 0.6, floor(height / 2) + deck[0].image.width * 0.026 * 0.6, deck[0].image.width * 0.6, deck[0].image.height * 0.6);
  deck[0].show();
  ...
  if (moveCounter % 2 == ENEMY) {
    image(trackerE, width / 2, height / 2, 35, 35);
  } else {
    image(trackerP, width / 2, height / 2, 35, 35);
  }
  ...
  if (gamestate == PLAYING) {
    ...
    textSize(15);
    text("CREATED BY:", width / 2, height - 48);
    text("@KROULITSCH", width / 2 + 10, height - 20);

    image(ig, floor(width / 2) - 80, floor(height) - 28);
    ...
    noStroke();
    fill(255);
    text("PRŠÍ", width / 2, 70);
    textSize(22);
    text("VÍTEJTE V KLASICKÉ", width / 2, 110);
    text("KARETNÍ HŘE PRŠÍ!", width / 2, 150);
    ...
    text("VÁŠ TAH JE ZNAČEN        ,", width / 2, 225);
    text("SOUPEŘŮV TAH JE ZNAČEN        .", width / 2, 250);
    text("PRO LÍZNUTÍ KARTY KLIKNĚTE NA BALÍČEK.", width / 2, 275);
    text("PRO LÍZNUTÍ NA 7 KLIKNĚTE NA         .", width / 2, 300);
    text("PRO STÁNÍ NA ESO KLIKNĚTE NA          .", width / 2, 325);
    text("DÁMA SE DÁ POLOŽIT NA JAKOUKOLIV KARTU.", width / 2, 350);
    text("PO POLOŽENÍ DÁMY ZVOLTE POŽADOVANÝ ZNAK.", width / 2, 375);

    textSize(30);
    //text("PŘIPRAVENI? TAK JDEME NA TO!!!", width / 2, 670);

    image(tp, floor(width / 2) + 65.5, 217 + 0.5, 25, 25);
    image(te, floor(width / 2) + 85.5, 242 + 0.5, 25, 25);
    image(draw2, floor(width / 2) + 105.5, 292 + 0.5, 30, 30);
    image(skipImg, floor(width / 2) + 105.5, 317 + 0.5, 30, 30);
...
function useCard(hand, n) {
  ...
  if (canUse(card)) {
    ...
    if (gamedeck.length > 0) {
      gamedeck[gamedeck.length - 1].setPos(width - 70, floor(height / 2));
      gamedeck[gamedeck.length - 1].flip();
      deck.unshift(gamedeck[gamedeck.length - 1]);
    }
    gamedeck = [];
    gamedeck.push(card);
    card.setPos(70, floor(height / 2));
    if (!card.visible) {
      card.flip();
    }
    if (card.value == 7) {
...
function mouseClicked() {
  ...
  if(playButton.clicked() || musicButton.clicked() || SFXButton.clicked()) {
    return;
  }
  if (gamestate == PLAYING) {
    ...
    if (moveCounter % 2 == PLAYER) {
      ...
      if (deck[0].clicked() && hand1.length < 19) {
        drawCard(hand1, true);
        drawSound.play();
      } else if (hand1.length >= 19) {
        errorSound.play();
      }
    }
  }
...
function restart(button) {
  ...
  for (let c of hand1) {
    ...
    c.setPos(width - 70, floor(height / 2));
    deck.push(c);
  }
