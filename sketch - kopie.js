...
function setup() {
  ...
  playButton = new Button(floor(width / 2), 740, "./images/play.png");
  ...
  for (let i = hand1.length - 1; i >= 0; i--) {
    let c = hand1[i];
    c.setPos(100 + i * 50, height - 150);
  }
  for (let i = hand2.length - 1; i >= 0; i--) {
    let c = hand2[i];
    c.setPos(100 + i * 50, 150);
  }
...
function draw() {
  ...
  image(cardback, floor(width - 300) - 9.5, floor(height / 2) + 10.5);
  image(cardback, floor(width - 300) - 4.5, floor(height / 2) + 5.5);
  deck[0].show();
  ...
  if (moveCounter % 2 == ENEMY) {
    image(trackerE, 80.5, height / 2);
  } else {
    image(trackerP, 80.5, height / 2);
  }
  ...
  if (gamestate == PLAYING) {
    ...
    noStroke();
    fill(255);
    text("PRŠÍ", width / 2, 130);
    textSize(30);
    text("VÍTEJTE V KLASICKÉ KARETNÍ HŘE PRŠÍ!", width / 2, 200);
    ...
    text("PRAVIDLA HRY:", width / 2, 280);
    text("VÁŠ TAH JE ZNAČEN        , SOUPEŘŮV TAH JE ZNAČEN        .", width / 2, 330);
    text("PRO LÍZNUTÍ KARTY KLIKNĚTE NA      .", width / 2, 380);
    text("PRO LÍZNUTÍ NA 7 KLIKNĚTE NA       .", width / 2, 430);
    text("PRO STÁNÍ NA ESO KLIKNĚTE NA        .", width / 2, 480);
    text("DÁMA SE DÁ POLOŽIT NA JAKOUKOLIV KARTU.", width / 2, 530);
    text("PO POLOŽENÍ DÁMY ZVOLTE POŽADOVANÝ ZNAK.", width / 2, 580);

    textSize(30);
    text("PŘIPRAVENI? TAK JDEME NA TO!!!", width / 2, 670);

    image(tp, floor(width / 2) - 75.5, 320 + 0.5, 40, 40);
    image(te, floor(width / 2) + 340.5, 320 + 0.5, 40, 40);
    image(cardback, floor(width / 2) + 225.5, 370 + 0.5, 29, 40);
    image(draw2, floor(width / 2) + 213.5, 420 + 0.5, 40, 40);
    image(skipImg, floor(width / 2) + 213.5, 470 + 0.5, 40, 40);
